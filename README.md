<div align="center">

<img src="https://capsule-render.vercel.app/api?type=venom&color=0:0F2027,50:203A43,100:2C5364&height=250&section=header&text=PRACHI%20PATIL&fontSize=65&fontColor=00F7FF&animation=twinkling&fontAlignY=35&desc=Aspiring%20DevOps%20Engineer%20•%20Cloud%20Engineer%20•%20AWS%20Enthusiast&descAlignY=58&descSize=18&descColor=B389FF"/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=24&pause=1200&color=00F7FF&center=true&vCenter=true&width=900&lines=Aspiring+DevOps+Engineer;Building+Production+Ready+Infrastructure;AWS+%7C+Docker+%7C+Kubernetes;Infrastructure+as+Code;CI%2FCD+Automation;Linux+%7C+Cloud+%7C+DevOps"/>

</div>

---
## 👩‍💻 About Me

I’m an aspiring **DevOps & Cloud Engineer** passionate about building reliable, scalable, and automated infrastructure.

Rather than simply developing applications, I enjoy designing the systems that power them—from containerization and CI/CD pipelines to cloud deployments and infrastructure automation.

I believe the best way to learn DevOps is by building real-world projects instead of endlessly following tutorials. Every project helps me understand how modern production systems are deployed, monitored, and maintained.

Alongside DevOps, I also have experience with **Python**, giving me a broader understanding of the complete software development lifecycle.

### 🚀 What I'm Currently Focused On

- ☁️ Cloud Infrastructure with AWS
- 🐳 Docker & Containerization
- ☸️ Kubernetes & Container Orchestration
- 🔄 CI/CD Pipelines with GitHub Actions & Jenkins
- 🏗️ Infrastructure as Code using Terraform
- 🐧 Linux Administration & Bash Scripting
- 📊 Monitoring & Observability with Prometheus & Grafana

> **"Automate repetitive tasks. Build reliable systems. Keep learning."**

---
## 🛠️ Tech Stack

### 💻 Programming Languages

<p align="center">
  <img src="https://skillicons.dev/icons?i=java,python,bash&theme=dark"/>
</p>

---

### ☁️ Cloud

<p align="center">
  <img src="https://skillicons.dev/icons?i=aws&theme=dark"/>
</p>

---

### 🚀 DevOps & Infrastructure

<p align="center">
  <img src="https://skillicons.dev/icons?i=docker,kubernetes,jenkins,terraform,linux&theme=dark"/>
</p>

---

### 🗄️ Databases

<p align="center">
  <img src="https://skillicons.dev/icons?i=mysql,mongodb&theme=dark"/>
</p>

---

### 🔧 Version Control

<p align="center">
  <img src="https://skillicons.dev/icons?i=git,github&theme=dark"/>
</p>

---

### 📚 Currently Learning

<p align="center">
  <img src="https://skillicons.dev/icons?i=prometheus,grafana&theme=dark"/>
</p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=helm,ansible&theme=dark"/>
</p>

<p align="center">
<b>Helm</b> • <b>Ansible</b> • <b>ArgoCD</b>
</p>

---
## 🚀 Current Focus

- 🐳 Building Docker projects and mastering containerization
- ☸️ Learning Kubernetes through hands-on labs and deployments
- ☁️ Deploying scalable applications on AWS
- 🔄 Creating CI/CD pipelines with GitHub Actions & Jenkins
- 🏗️ Automating infrastructure using Terraform
- 🐧 Strengthening Linux & Bash scripting skills
- 📊 Exploring monitoring with Prometheus & Grafana
- 🚀 Building end-to-end production-ready DevOps projects
---


## 📊 GitHub Stats

<p align="center">
  <img src="https://streak-stats.demolab.com?user=Pprachi1611&theme=tokyonight&hide_border=true&cache_seconds=86400&date_format=%5Bj%20M%5D"/>
</p>


---

## 📈 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Pprachi1611&theme=github-dark&hide_border=true"/>
</p>



---

## 🌐 Connect With Me

<p align="center">
  <a href="https://www.linkedin.com/in/pprachi16"><img src="https://img.icons8.com/color/96/linkedin.png"/></a>
  <a href="https://github.com/Pprachi1611"><img src="https://img.icons8.com/ios-glyphs/90/github.png"/></a>
</p>

---

⭐ *"Turning data into intelligence, and systems into impact."*
